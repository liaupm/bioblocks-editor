...
AppController = function() {
  ...
  this.tabMap[AppController.MY_TAB] =
      document.getElementById('my_space_tab');
...
AppController.prototype.onTab = function() {
  ...
  var mySpaceTab = this.tabMap[AppController.MY_TAB];
  ...
  if (this.selectedTab == AppController.EXPORTER) {
    ...
    FactoryUtils.hide('my_space_tab');
    // Show exporter tab.
    ...
    this.workspaceFactoryController.setBlockLibCategory(categoryXml,
        blockTypes);
  } else if (this.selectedTab == AppController.MY_TAB) {
    BlocklyDevTools.Analytics.onNavigateTo('BlockFactory');
    ...
    FactoryUtils.hide('workspaceFactoryContent');
    // Show workspace factory container.
    FactoryUtils.show('my_space_tab');
    // Update block library category.
  }
...
AppController.prototype.assignExporterClickHandlers = function() {
  var self = this;
/*
  document.getElementById('button_my_save').addEventListener('click',
      function() {
        self.save();
      });

  document.getElementById('button_my_reload').addEventListener('click',
      function() {
        self.restore();
      });
*/
...
//ADDED BY BUHORL -COPIED FROM: https://samadhipoornima.wordpress.com/2016/06/21/play-around-with-blockly-save-and-restore-the-workspace/

AppController.prototype.save = function(){
  if(typeof(Storage)!=="undefined")
  {
    var xml = Blockly.Xml.workspaceToDom( Blockly.mainWorkspace );
    localStorage.setItem(document.getElementById("projectName").value,Blockly.Xml.domToText( xml ));
...
AppController.prototype.restore = function(){
  Blockly.mainWorkspace.clear()
  var nameOfTheProject = document.getElementById("projectName").value;
  if(typeof(Storage)!=="undefined"){
